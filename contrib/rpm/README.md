...
reference the release tarball available from https://geekcash.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Bitcoin git repository](https://github.com/geekcash/geekcash) but are not part of
...
Currently only `Source10` is used, for the example `geekcash.conf` file.
...
in the geekcash git repository. At present only an SVG version of the Bitcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the geekcash
...
The `geekcash.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `geekcash-server` package. At this point, most RPM based
...
The `geekcash-server` package should have SELinux support. How to properly do
...
[Ringing Liberty](https://www.ringingliberty.com/geekcash/). He has been
...
4. I renamed the `geekcash` package that contains the Qt GUI to `geekcash` as
that appears to be how the general population refers to it, in contrast to
`geekcash-xt` or `geekcash-classic`. I wanted to make sure the general population
