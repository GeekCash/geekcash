...
###   bitcoincore.org and geekcash.org and compares them.
...
HOST2="https://geekcash.org"
BASEDIR="/bin/"
VERSIONPREFIX="geekcash-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'geekcash-',
...
if [ $? -ne 0 ]; then
   echo "geekcash.org failed to provide signature file, but bitcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "geekcash.org and bitcoincore.org signature files were not equal?"
