...
*
*   Why is this needed?
*   - In GeekCash some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockPayeeValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
    ...
    if (nBlockHeight < consensusParams.nMasternodePaymentsStartBlock) {
        //there is no budget data to use to check anything, let's just accept the longest chain
        if (fDebug) LogPrintf("IsBlockPayeeValid -- MasternodePayment haven't started\n");
        return true;
    }
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f GEEK\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
