...
const string strMessageMagic = "GeekCash Signed Message:\n";
...
// double NetworkHash(int lookup, int height) {
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    // end year 2036 at block 9460800 ~ 3,6B coins 
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (fCheckMerkleRoot) {
        ...
        if (block.hashMerkleRoot != hashMerkleRoot2){
             //LogPrintf("CheckBlock: hashMerkleRoot mismatch hashMerkleRoot2: %s:%s\n", hashMerkleRoot2, block.hashMerkleRoot);  
            //printf("CheckBlock: %s\n", block.ToString().c_str());          
             return state.DoS(100, error("CheckBlock(): hashMerkleRoot mismatch %s:%s\n%s", block.hashMerkleRoot.ToString(), hashMerkleRoot2.ToString(), block.ToString()),
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(GEEK): transaction %s conflicts with transaction lock %s",
